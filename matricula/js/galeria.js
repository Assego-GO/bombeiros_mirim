// galeria.js - VERSÃO COMPLETA COM UTF-8 PARA BRASIL
(function() {
    'use strict';
    
    console.log('🎯 Galeria JS - Versão UTF-8 Brasil Carregada');
    ...
    class SistemaGaleria {
        ...
        configurarLocaleBrasil() {
            ...
            console.log('🇧🇷 Configurações brasileiras aplicadas na galeria');
        }
        ...
        formatarData(dateString) {
            ...
            try {
                ...
                return this.formatadorData.format(data) + ' às ' + this.formatadorHora.format(data).slice(0, 5);
            } catch (error) {
        ...
        async carregarGalerias() {
            ...
            try {
                ...
                container.innerHTML = `<div class="galeria-alert galeria-alert-danger">Erro de conexão: ${this.sanitizarTexto(error.message)}</div>`;
            }
        }
        ...
        async enviarGaleria(e) {
            ...
            // Validações básicas
            if (!titulo || !atividade) {
                this.showMessage('mensagem-galeria', 'Preencha todos os campos obrigatórios.', 'danger');
                return;
            }
            ...
            if (!isAdmin && !turmaId) {
                ...
                this.showMessage('mensagem-galeria', 'Selecione uma turma válida.', 'danger');
                return;
            }
            ...
            let statusMsg;
            if (isAdmin && !turmaId) {
                statusMsg = `📂 Criando galeria geral (sem turma) com ${this.formatarNumero(this.arquivosSelecionados.length)} arquivo(s)...`;
            } else if (isAdmin && turmaId) {
                statusMsg = `📂 Admin criando galeria para turma específica com ${this.formatarNumero(this.arquivosSelecionados.length)} arquivo(s)...`;
            } else {
                statusMsg = `📂 Enviando galeria com ${this.formatarNumero(this.arquivosSelecionados.length)} arquivo(s)...`;
            }
            ...
            try {
                ...
                if (data.success) {
                    this.showMessage('mensagem-galeria', this.sanitizarTexto(data.message || '✅ Galeria criada com sucesso!'), 'success');
                    setTimeout(() => {
                        this.fecharModalNovaGaleria();
                        this.carregarGalerias();
                    }, 2000);
                ...
                this.showMessage('mensagem-galeria', `❌ Erro: ${this.sanitizarTexto(error.message)}`, 'danger');
            } finally {
                if (btnEnviar) {
            ...
            try {
                ...
                if (data.success) {
                    ...
                    if (galeria.arquivos && galeria.arquivos.length > 0) {
                        html += `
                            <div class="galeria-detalhes">
                                <h4>Fotos e Vídeos (${this.formatarNumero(galeria.arquivos.length)})</h4>
                            ...
                            html += `
                                <div class="galeria-arquivo-item" onclick="abrirLightbox('${caminhoCorreto}', '${arquivo.tipo_arquivo}', '${this.sanitizarTexto(arquivo.nome_original)}')">
                                    ...
                                    <div class="galeria-arquivo-info">
                                        ...
                                        <div>${isVideo ? 'Vídeo' : 'Imagem'} • ${this.formatarTamanho(arquivo.tamanho)}</div>
                                    </div>
                                </div>
                            `;
                        });
        ...
        async excluirGaleria(galeriaId) {
            ...
            try {
                ...
                if (data.success) {
                    alert(this.sanitizarTexto(data.message || '✅ Galeria excluída com sucesso!'));
                    this.carregarGalerias();
                } else {
        ...
        // [Resto dos métodos: configurarUploadArquivos, adicionarArquivos, etc. - mantidos iguais mas com sanitização]
        ...
        adicionarArquivos(novosArquivos) {
            console.log('➕ Adicionando', novosArquivos.length, 'arquivos aos', this.arquivosSelecionados.length, 'existentes');
        ...
        atualizarInputEPreview() {
            ...
            if (inputArquivos && this.arquivosSelecionados.length > 0) {
                ...
                console.log('🔄 Input atualizado com', this.arquivosSelecionados.length, 'arquivos');
        ...
        removerArquivo(indexToRemove) {
            console.log('🗑️ Removendo arquivo index:', indexToRemove, 'de', this.arquivosSelecionados.length);
            
            this.arquivosSelecionados.splice(indexToRemove, 1);
            this.atualizarInputEPreview();
            
            console.log('✅ Arquivo removido. Restantes:', this.arquivosSelecionados.length);
        }
        ...
        validarArquivos() {
            ...
            if (this.arquivosSelecionados.length > maxFiles) {
                this.showMessage('mensagem-galeria', `Máximo de ${this.formatarNumero(maxFiles)} arquivos permitidos. Você selecionou ${this.formatarNumero(this.arquivosSelecionados.length)}.`, 'danger');
                return false;
            }
            ...
            for (let i = 0; i < this.arquivosSelecionados.length; i++) {
                ...
                if (file.size > maxSize) {
                    this.showMessage('mensagem-galeria', `Arquivo "${this.sanitizarTexto(file.name)}" é muito grande. Máximo 50MB por arquivo.`, 'danger');
                    return false;
                }
                
                if (!allowedTypes.includes(file.type)) {
                    this.showMessage('mensagem-galeria', `Arquivo "${this.sanitizarTexto(file.name)}" não é um tipo permitido.`, 'danger');
                    return false;
                }
            }
    ...
    function inicializar() {
        console.log('🚀 Inicializando módulo de galeria com UTF-8...');
    ...
    window.abrirLightbox = function(caminho, tipo, nomeOriginal) {
        ...
        if (tipo === 'video') {
            ...
            mediaContainer.innerHTML = `
                <img src="${caminho}" alt="${nomeSeguro}" onerror="console.error('❌ Erro ao carregar imagem:', '${caminho}')">
                <p style="color: white; margin-top: 10px;">${nomeSeguro}</p>
            `;
        }
    ...
    console.log('✅ Galeria JS COMPLETO - Versão UTF-8 Brasil sem Interferências!');
